    ...
    for (int j = 0; j < N; j++) {
      ...
      for (int k = 0; k < N; k += 4) {
        ...
        if (k + 3 < N) {
          a_vec = *((v4sf *)&A[i * N + k]);
          b_vec = *((v4sf *)&B[k * N + j]);
          // Нужно перемножить и сложить поэлементно
          mul_vec = a_vec * b_vec;
...
float norm1(float *A, int N) {
  ...
  for (int j = 0; j < N; j++) {
    float sum = 0.0f;
    // Векторизация суммирования по столбцу
    int i;
    for (i = 0; i <= N - 4; i += 4) {
...
float norm_inf(float *A, int N) {
  ...
  for (int i = 0; i < N; i++) {
    float sum = 0.0f;
    // Векторизация суммирования по строке
    int j;
    for (j = 0; j <= N - 4; j += 4) {
      v4sf a_vec = *((v4sf *)&A[i * N + j]);
      v4sf abs_vec;
